...
import { coreLightMinus1, coreNeutral4 } from '../../colors';
...
export default class SimpleTabs extends React.Component {
  ...
  handleClick(e, href, tabIndex) {
    ...
    onItemSelected && onItemSelected(href, tabIndex);
  ...
  render() {
    ...
    const currentTab = selectedTabIndex || initialTabIndex || 0;

    return (
      <StyledCarousel withArrow={false} className={className}>
        {tabs.map(({ label, disabled, href }, i) => {
          if (currentTab === i) return <TabSelected key={href}>{label}</TabSelected>;
          if (disabled === true) return <TabInactive key={href}>{label}</TabInactive>;
          return (
            <TabActive key={href} onClick={e => this.handleClick(e, href, i)} href={href}>
              {label}
            </TabActive>
          );
        })}
